        ...
        if (check == false) {
            ...
            for (var j = 0; j < links.length; j++) {
                if (!usedLinks.has(links[j])) {
                    var feedLink = links[j]; // Use the jth feed link that is not already used
                    console.log("Redirecting link: ", linktag[i].href, " to: ", feedLink);
                    linktag[i].href = feedLink + setting.path + aesCrypto.encrypt(convertstr(linktag[i].href), convertstr('root'));
