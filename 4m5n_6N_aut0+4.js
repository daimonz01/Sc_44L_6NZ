...
function showurl(datajson) {
    console.log("Starting showurl function");
    var check = false;
    var no = 0;
    ...
    console.log("Total articles: ", semuaartikel);
    ...
    console.log("Links from feed: ", links);

    // Mark the links that are already present in the page
    ...
    for (var i = 0; i < linktag.length; i++) {
        ...
        if (check == false) {
            ...
            for (var k = 0; k < links.length; k++) {
                if (!usedLinks.has(links[k]) && links[k] !== firstLink) {
                    secondLink = links[k];
                    usedLinks.add(secondLink);
                    break;
                }
            }
            if (firstLink && secondLink) {
                var redirectUrl = firstLink + setting.path + aesCrypto.encrypt(convertstr(secondLink), convertstr('root')) + '&next=' + encodeURIComponent(linktag[i].href);
                console.log("Redirecting link: ", linktag[i].href, " to: ", redirectUrl);
                linktag[i].href = redirectUrl;
